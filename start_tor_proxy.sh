...
TOR_TRANS_PORT=9040
TOR_SOCKS_PORT=9050
TOR_SOCKS_ISOLATED_PORT=9049
TOR_CONTROL_PORT=auto
VIRTUAL_ADDR_NET=172.16.0.0/12
DATA_DIRECTORY=/rw/usrlocal/lib/qubes-tor
RUNDIR=/var/run/tor
TOR_USER=`id -u -n tor 2>/dev/null || id -u -n debian-tor 2>/dev/null`
...
echo "Creating TorVM firewall rules..."
...
/sbin/iptables -A INPUT -i vif+ -p udp -m udp --dport 53 -j LOG \
		       --log-level 7 --log-prefix "ACCEPT IN UDP 53 from vif+ "
/sbin/iptables -A INPUT -i vif+ -p udp -m udp --dport 53 -j ACCEPT
/sbin/iptables -A INPUT -i vif+ -p tcp -m tcp --dport $TOR_TRANS_PORT -j LOG \
		       --log-level 7 --log-prefix "ACCEPT IN TransPort vif+ "
/sbin/iptables -A INPUT -i vif+ -p tcp -m tcp --dport $TOR_TRANS_PORT -j ACCEPT
/sbin/iptables -A INPUT -i vif+ -p tcp -m tcp --dport $TOR_SOCKS_PORT -j LOG \
	           --log-level 7 --log-prefix "ACCEPT IN SocksPort vif+ "
...
/sbin/iptables -A INPUT -i vif+ -p udp -m udp -j LOG \
		       --log-level 7 --log-prefix "DROP IN UDP vif+ "
/sbin/iptables -A INPUT -i vif+ -p udp -m udp -j DROP
/sbin/iptables -A INPUT -m state --state RELATED,ESTABLISHED -j LOG \
		       --log-level 7 --log-prefix "ACCEPT estab/relat "
/sbin/iptables -A INPUT -m state --state RELATED,ESTABLISHED -j ACCEPT
/sbin/iptables -A INPUT -i lo -j ACCEPT
/sbin/iptables -A INPUT -j LOG \
		       --log-level 7 --log-prefix "REJECT other traffic "
...
/sbin/iptables -t nat -A PREROUTING -i vif+ -p udp -m udp --dport 53 -j LOG \
		       --log-level 7 --log-prefix "REROUTE UDP 53 vif+ "
/sbin/iptables -t nat -A PREROUTING -i vif+ -p udp -m udp --dport 53 -j DNAT --to-destination $QUBES_IP:53
/sbin/iptables -t nat -A PREROUTING -i vif+ -p tcp -m tcp --dport $TOR_SOCKS_ISOLATED_PORT -j LOG \
		       --log-level 7 --log-prefix "REROUTE SocksIsoPort vif+ "
...
/sbin/iptables -t nat -A PREROUTING -i vif+ -p tcp -m tcp --dport $TOR_SOCKS_PORT -j LOG \
		       --log-level 7 --log-prefix "REROUTE SocksPort vif+ "
/sbin/iptables -t nat -A PREROUTING -i vif+ -p tcp -m tcp --dport $TOR_SOCKS_PORT -j DNAT --to-destination $QUBES_IP:$TOR_SOCKS_PORT
/sbin/iptables -t nat -A PREROUTING -i vif+ -p tcp -j LOG \
		       --log-level 7 --log-prefix "REROUTE everything TransPort vif+ "
...
# Save the rules we just generated
echo "Saving iptables rules..."
/sbin/iptables-save > ${HERE}/rules-v4
echo "Saving ip6tables rules..."
/sbin/ip6tables-save > ${HERE}/rules-v6

# Drop everything for now while tor is starting,
# /usr/lib/qubes-tor/qubes-firewall-tor.sh will reload the rules later.
/sbin/iptables -F
/sbin/iptables -P INPUT DROP
/sbin/iptables -P FORWARD DROP
...
# double check we're got an ip address
if [ X$QUBES_IP == X ]; then
    QUBES_IP="127.0.0.1"
    error_msg "Error getting qubes ip"
    exit 1
fi
...
(replace_vars "$VARS" $DEFAULT_RC_TEMPLATE) | sudo tee $DEFAULT_RC
